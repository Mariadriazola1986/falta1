        ...
        success: function (response) {
                $.each(response, function(){
                    $("#tablaUsuariosRegistrados").append($("<tr></tr>").append(
                        ...
                        $("<td></td>").append(
                            $("<button></button>").attr("id", this.ID_USUARIO).attr("class", 'btn btn-info').text("Dar de alta al usuario"),
                            $("<button></button>").attr("id", this.ID_USUARIO).attr("class", 'btn btn-info').attr("data-target", '#modalRegistro').text("Modificar usuario"),
                            $("<button></button>").attr("id", this.ID_USUARIO).attr("class", 'btn btn-info').text("Dar de baja al usuario"))
                        ));
                })
        },
