	...
	({
		...
		success:  function (response) {
			if (response.error=="NO") {
				$("body").append('<div class="modal fade" id="hoyJuegoPartido" role="dialog"><div class="modal-dialog modal-lg"><div class="modal-content"><div class="modal-header"><button type="button" class="close" data-dismiss="modal">&times;</button><h4 class="modal-title">Hoy jugas estos partidos:</h4></div><div class="modal-body"><div class="panel panel-default"><div class="panel-body"><div class="table-responsive"><table class="table"><thead><tr><th>Fecha</th><th>Hora de inicio</th><th>Hora de fin</th><th>Tipo de Futbol</th><th>jugadores actuales</th></tr></thead><tbody id="QueJuegoHoy"><tr></tr></tbody></table></div></div></div></div><div class="modal-footer"><button type="button" class="btn btn-default" data-dismiss="modal">Cerrar</button></div></div></div></div>');
				$("#QueJuegoHoy").empty();
				$("#hoyJuegoPartido").modal("show");
				$.each(response.datos, function (){
					
					$("#QueJuegoHoy").append('<tr class="success"><td>'+this.FECHA+'</td><td>'+this.HORA+'</td><td>'+this.HORA_FIN+
