<?php
    ...
    if(isset($_POST["nombre"]) && isset($_POST["apellido"]) && isset($_POST["dni"]) && isset($_POST["telefono"])&& isset($_POST["idpartido"]))
    {
        if (!empty($_POST["nombre"])&&!empty($_POST["apellido"])&&!empty($_POST["dni"])&&!empty($_POST["telefono"])) {
                        ...
                        $conn = getConnection();
                        $sqlyaseanotoalpartido="select I.NOMBRE,I.DNI from invitados as I where exists (select * from usuarios_juegan_partidos as Us where Us.ID_PARTIDO=:idpartido AND I.ID_INVITADO=Us.ID_INVITADO AND I.DNI=:dni)";
                        $resultadoseanoto=$conn->prepare($sqlyaseanotoalpartido);
                        $resultadoseanoto->execute(array(":idpartido"=>$_POST["idpartido"],":dni"=>$_POST["dni"]));
                        $trajoalgo=$resultadoseanoto->fetchAll(PDO::FETCH_ASSOC);
                        if ($trajoalgo) {
                            $resultados_de_validacion['error']="Vos ya te anotaste a este partido";
                        }
                        else{
